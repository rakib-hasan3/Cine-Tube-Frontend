...
import { User, LogOut, ChevronDown, Bell, Menu, X } from "lucide-react"; // Menu, X যোগ করা হয়েছে
import { useState } from "react";
import axiosInstance from "@/lib/axios";

export default function Navbar() {
    const { user, setUser } = useAuth();
    const [isOpen, setIsOpen] = useState(false); // প্রোফাইল ড্রপডাউন
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false); // মোবাইল সাইডবার
                        ...
                        {user ? (
                            <div className="flex items-center gap-4">
                                ...
                                <div className="relative">
                                    ...
                                    >
                                        <span className="hidden sm:inline text-sm font-bold text-white uppercase tracking-wider">{user.name.split(' ')[0]}</span>
